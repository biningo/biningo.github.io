...
```go
array := []int{1,2,3} //len(array)=3 cap(array)=3
...
//arr=arr[:11] //报错 超过cap的大小


//2
arr := [9]int{1,2,3,4,5,6,7,8,9}
arr2 :=arr[2:5:8]  //长度为 5-2=3  容量为8-2=6
log.Println(arr2[0])//3
log.Println(arr2) // 3 4 5
arr2=arr2[:4] //不能超过 6
log.Println(arr2) // 3 4 5 6



//3
a1:=[]int{1,2,3,4,5,6,7,8}
log.Println(len(a1),cap(a1)) //8 8
a2:=a1[1:3] //贡献底层空间 [2 3]  len=2  cap=8-1=7
log.Println(len(a2),cap(a2)) //2 7 
```
...
下面做一些小优化，忽略值，而是用索引来，这样就不用再开辟空间创建一个变量并且赋值给变量了

```go
//优化
arrStr:=[]string{"a","b","c","d"}
for key:=range arrStr{ //只是一个的话默认就是切片的索引
