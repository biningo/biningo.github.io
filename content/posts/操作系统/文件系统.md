...
内核会维护一个所有打开文件的一个 **打开文件表**，同时每个进程里面也有一个`fd`数组，**文件描述符fd** 其实就是一个整数，这个值就是进程自己打开文件表的一个数组下标索引，系统打开表最终会指向一个`inode`节点，fd其实只是在进程中标识一个该进程打开的文件，出了进程就没有意义了
...
https://zhuanlan.zhihu.com/p/143430585 【fd文件描述符表和inode】