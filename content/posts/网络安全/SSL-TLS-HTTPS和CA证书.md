...
- 加密信息，防止信息被篡改。通常中间人想要篡改信息体那么就必须连同摘要也一并篡改，中间人想要篡改摘要那么就必须使用发送者的私钥将篡改后的摘要进行再次加密，但是私钥只有发送者才有不会被泄露
...
验证数字签名的前提是要拿到对方的公钥，但是公钥在传输过程中可能会被中间人给替换，所以需要通过CA证书证明公钥是否属于对方，而CA证书的信息是被CA私钥加密过的，我们只需要用CA公钥进行解密即可拿到公钥，因为中间人没有CA的私钥所以无法篡改伪造证书内容，一旦篡改就会被发现。CA的公钥也需要上一级CA机构证书，顶级的CA则只能自己证明自己了，所以顶级CA必须足够权威
