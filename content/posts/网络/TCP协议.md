...
注意，A发送完回复之后还不能马上断开，还需要等一段时间，会启动一个定时器，如果在`TIME_WAIT`期间又都到了B发来的FIN则会继续回复ACK包然后重置定时器
...
并且等待的时间为`2MSL`，`MSL`这个为网络IP报文最大的生存时间，这样就可以接受所有之前还遗留在网络上的TCP报文，不至于对以后的同端口应用造成骚扰，超过这个时间之后网络上的报文肯定全部都挂了不存在了
...
TCP在数据包发出来的时候回启动一个定时器，如果定时器过了还没有收到对方的ACK那么就会重发，这就是RTO的用途

如果RTO太大，那么用户体验会非常不好，包丢弃了则需要过很久才察觉到才会进行重发，会造成很大的延迟，如果RTO太小，那么会造成很多无用的重发增加消耗，对方的ACK还没到就触发重发了
...
- https://yuanrengu.com/2020/77eef79f.html 【TCP握手和挥手】
- https://developer.aliyun.com/article/773014 【全连接和半连接】
- https://www.cnblogs.com/xiaolincoding/p/12995358.html 【全连接和半连接】
- https://www.huaweicloud.com/articles/b026587fb60bbb3f12202b9c30358eb1.html 【MSS】
- https://draveness.me/whys-the-design-tcp-segment-ip-packet 【为什么 TCP/IP 协议会拆分数据】
- http://c.biancheng.net/view/6427.html【TCP滑动窗口】
