...
由于初始的seq号是随机生成的，所以TCP回绕到0之后怎么继续保持字节序呢?

看下面文章分析不错:https://blog.csdn.net/hi_software/article/details/51768026

```c
static inline int before(__u32 seq1, __u32 seq2)
{
return (__s32)(seq1-seq2) < 0; //(__s32)表示强转为有符号的
}
```
...
并且时间戳还可以用于防止 **Sequence序列号回滚问题** ，也就是如果后面序列号回滚了，但是前面的包还没到达，那么则会发生错误，这个时候可以通过判断时间戳来识别两个TCP包，直接丢弃掉与最近接收到的报文段时间戳相差最远的那个报文段即可
